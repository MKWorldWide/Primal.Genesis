...
# Configuration
INSTANCE_ID="i-01f16ff979f0934b1"
SSH_KEY="$HOME/.ssh/serafina-key-new.pem"
SSH_USER="ec2-user"
...
echo "Deploying to AthenaCore-Server ($PUBLIC_IP)..."
...
# Set up the AthenaCore service
echo "Setting up AthenaCore service..."
sudo tee /etc/systemd/system/athenacore.service > /dev/null <<SERVICE_EOF
[Unit]
Description=AthenaCore Service
...
echo "Starting AthenaCore service..."
sudo systemctl daemon-reload
sudo systemctl enable athenacore
sudo systemctl restart athenacore

# Check service status
echo "AthenaCore service status:"
sudo systemctl status athenacore --no-pager
...
echo "Starting deployment on AthenaCore-Server..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@$PUBLIC_IP" "bash -s" <<< "$deploy_script"

echo "\nAthenaCore deployment completed!"
echo "You can access AthenaCore at: http://$PUBLIC_IP:8000"
